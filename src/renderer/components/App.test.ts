...
describe('App', () => {
  ...
  it('Ctrl+Shift+F does not throw when called before workspace is loaded', () => {
    new App();
    const event = new KeyboardEvent('keydown', {
      key: 'f',
      ctrlKey: true,
  ...
  it('Ctrl+F without Shift does not trigger search overlay (no explorer yet)', () => {
    new App();
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const event = new KeyboardEvent('keydown', { key: 'f', ctrlKey: true, shiftKey: false, bubbles: true });
    expect(() => document.dispatchEvent(event)).not.toThrow();
    spy.mockRestore();
  });

  it('Cmd+Shift+F does not throw when called before workspace is loaded', () => {
    new App();
    const event = new KeyboardEvent('keydown', {
      key: 'f',
      metaKey: true,
