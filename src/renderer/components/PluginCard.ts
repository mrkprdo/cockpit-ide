...
export class PluginCard {
  ...
  private initResize(): void {
    ...
    for (const [d, h] of Object.entries(handlers)) {
      h.addEventListener('mousedown', (e) => {
        ...
        this.updateResizeTooltip(e.clientX, e.clientY, this.opts.width, this.opts.height);
      });
    }
  ...
  private createResizeTooltip(): void {
    ...
    el.className = 'resize-tooltip';
    el.setAttribute('aria-hidden', 'true');
    document.body.appendChild(el);
    this.resizeTooltip = el;
  }

  private updateResizeTooltip(cx: number, cy: number, w: number, h: number): void {
    if (!this.resizeTooltip) return;
    const gu = (v: number) => Math.round(v / SNAP);
    this.resizeTooltip.textContent = `${gu(w)} × ${gu(h)}`;
    this.resizeTooltip.style.left = `${cx + 16}px`;
    this.resizeTooltip.style.top = `${cy + 16}px`;
  }

  private destroyResizeTooltip(): void {
