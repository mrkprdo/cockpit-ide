...
export class GitPlugin {
  ...
  private statusMsgEl: HTMLDivElement | null = null;
  private statusTimer: ReturnType<typeof setTimeout> | null = null;
  private stageAllBtn: HTMLButtonElement | null = null;
  private unstageAllBtn: HTMLButtonElement | null = null;
  ...
  private buildUI(): void {
    ...
    this.unstageAllBtn = document.createElement('button');
    this.unstageAllBtn.className = 'git-changes-action git-changes-all';
    this.unstageAllBtn.textContent = '− all';
    this.unstageAllBtn.title = 'Unstage all files';
    this.unstageAllBtn.style.display = 'none';
    this.unstageAllBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      await this.unstageAll();
    });
    this.stagedHeader.appendChild(this.unstageAllBtn);
    this.focusable(this.stagedHeader);
    ...
    this.stageAllBtn = document.createElement('button');
    this.stageAllBtn.className = 'git-changes-action git-changes-all';
    this.stageAllBtn.textContent = '+ all';
    this.stageAllBtn.title = 'Stage all files';
    this.stageAllBtn.style.display = 'none';
    this.stageAllBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      await this.stageAll();
    });
    this.unstagedHeader.appendChild(this.stageAllBtn);
    this.focusable(this.unstagedHeader);
    ...
    this.statusMsgEl = document.createElement('div');
    this.statusMsgEl.className = 'git-op-status';
    this.statusMsgEl.style.display = 'none';
    commitBar.appendChild(this.statusMsgEl);
  ...
  private async stageAll(): Promise<void> {
    ...
    for (const f of this.unstagedFiles) {
      await window.electronAPI?.git.stage(this.wsPath, f.path);
    }
    this.selectedFilePath = null;
    this.diffContent = '';
    this.renderDiff();
  ...
  private async unstageAll(): Promise<void> {
    for (const f of this.stagedFiles) {
      await window.electronAPI?.git.unstage(this.wsPath, f.path);
    }
    this.selectedFilePath = null;
    this.diffContent = '';
    this.renderDiff();
