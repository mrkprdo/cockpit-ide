...
export class PanelController {
  ...
  openPanel(node: SpecNode): void {
    ...
    try {
      ...
      this.host.panelInner.innerHTML =
        ...
        '<div style="font-size:10px;opacity:0.85;word-break:break-all">' + esc(msg) + '</div></div>';
    }
  }
  ...
  renderSettingsContent(): void {
    ...
    const levelChips = (lvl: number) =>
      `<button class="sm-cycle-level" data-level="${lvl}" style="background:none;border:1px solid ${lvl === cycles.cycleLevel ? 'var(--accent)' : 'var(--border)'};color:${lvl === cycles.cycleLevel ? 'var(--accent)' : 'var(--tertiary)'};border-radius:5px;padding:2px 10px;font-family:inherit;font-size:10px;font-weight:700;cursor:pointer;transition:color 0.12s,border-color 0.12s" title="Level ${lvl}: ${lvl === 1 ? 'cycle nodes only' : lvl === 2 ? '+ immediate neighbors' : '+ 2-hop neighbors'}">${lvl}</button>`;
    ...
    this.host.panelInner.innerHTML =
      ...
      `<div id="sm-isolated-toggle" style="width:36px;height:20px;border-radius:10px;background:${isolatedToggleBg};cursor:pointer;position:relative;transition:background 0.15s;flex-shrink:0" role="switch" aria-checked="${cycles.isolatedMode}">` +
  ...
  renderPanelContent(node: SpecNode): void {
    ...
    this.host.panelHeaderEl.innerHTML =
      ...
      `<span style="font-size:14px;font-weight:700;color:var(--primary);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escF(node.name)}</span>` +
    ...
    const identity =
      ...
      `<div class="sm-panel-row" style="font-size:11px;cursor:pointer;color:var(--accent)" data-open-file="${escF(fullPath)}">${escF(node.specFile)}</div>` +
      (sourceFullPath ? `<div class="sm-panel-label" style="margin-top:8px">${fileLabel}</div><div class="sm-panel-row" style="font-size:11px;cursor:pointer;color:var(--accent)" data-open-file="${escF(sourceFullPath)}">${escF(node.sourceFile)}</div>` : '') +
      (parentNode ? `<div class="sm-panel-label" style="margin-top:8px">UI SPEC FOR</div><div class="sm-panel-row" style="font-size:12px;cursor:pointer;color:${colorVar}" data-goto="${escF(parentNode.id)}">${escF(parentNode.name)}</div>` : '') +
      (uiChild ? `<div class="sm-panel-label" style="margin-top:8px">UI SPEC</div><div class="sm-panel-row" style="font-size:12px;cursor:pointer;color:${colorVar}" data-goto="${escF(uiChild.id)}">${escF(uiChild.name)}</div>` : '') +
      `</div>`;

    const desc = data.description
      ? `<div class="sm-panel-section"><div class="sm-panel-label">DESCRIPTION</div><div style="font-size:12px;color:var(--secondary);line-height:1.6;word-break:break-word">${escF(data.description)}</div></div>`
      : '';
    ...
    const ipcSection = ipc.length
      ? `<div class="sm-panel-section"><div class="sm-panel-label">IPC CHANNELS (${ipc.length})</div>` +
        ipc.map(ch => `<div style="font-size:12px;color:var(--tertiary);padding:3px 0">${escF(ch)}</div>`).join('') +
        `</div>`
      : '';
    ...
    const declaredDepsSection = declaredDeps
      ? `<div class="sm-panel-section"><div class="sm-panel-label">DECLARED DEPS (${Object.keys(declaredDeps).length})</div>` +
        Object.entries(declaredDeps).map(([k, v]) =>
          ...
          `<span style="color:var(--tertiary);word-break:break-all;font-size:10px">${escF(v)}</span></div>`
        ).join('') +
        `</div>`
      : '';
