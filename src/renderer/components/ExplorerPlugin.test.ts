...
describe('ExplorerPlugin', () => {
  ...
  it('openSearch calls ensureExplorer and creates searchOverlay lazily', async () => {
    ...
    expect(dev.searchOverlay).toBeTruthy();
    const ov = document.querySelector('.search-overlay');
    expect(ov?.classList.contains('open')).toBe(true);
  });

  it('openSearch opens file and reveals tree when search result is selected', async () => {
    (mockElectronAPI.fs.readDir as any).mockImplementation(async (dir: string) => {
      if (dir === '/test/ws') return [{ name: 'file.ts', isDirectory: false }];
      return [];
    });
    ...
    // Simulate selecting a search result (fires the onSelect callback)
    ...
    // File should be selected in the tree
    const el = container.querySelector('[data-path="/test/ws/file.ts"]');
    expect(el).toBeTruthy();
    expect(el!.classList.contains('is-file-selected')).toBe(true);
  });
