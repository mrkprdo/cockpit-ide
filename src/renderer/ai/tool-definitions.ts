...
function truncate(content: string, length = READABLE_LENGTH): string {
  return content.length > length ? content.slice(0, length) + '\n[truncated]' : content;
}
...
export const refreshSpecsmapTool: ToolDefinition<typeof GetCanvasStateArgs> = {
  name: 'refresh_specsmap',
  description: 'Reload the SpecsMap graph from the current spec files on disk.',
  parameters: GetCanvasStateArgs,
  execute: (_args, ctx) => {
    ctx.cockpit.refreshSpecsMap();
    return 'SpecsMap refresh triggered';
  },
};

export const regenerateSpecsmapTool: ToolDefinition<typeof GetCanvasStateArgs> = {
  ...
  parameters: GetCanvasStateArgs,
  execute: async (_args, ctx) => {
    await ctx.cockpit.regenerateSpecs();
