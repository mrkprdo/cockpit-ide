...
export class IdeServer {
      ...
      ws.on('message', (raw: Buffer) => {
        try {
          ...
          if (msg.method === 'initialize' && msg.id != null) {
            if (msg.params?.authToken !== this.authToken) {
          ...
          } else if (initialized && msg.method === 'openFile' && msg.params?.filePath) {
  ...
  updateEditorState(state: EditorSelectionState | null): void {
    ...
    // The renderer sends this over `ide:editorState` with a compile-time-only
    // type — a malformed payload (e.g. filePath not a string) would otherwise
    // throw inside path.relative() and crash the whole main process, since
    // this handler is fired via ipcMain.on with no reply to catch on.
    if (!state || typeof state.filePath !== 'string') {
      msg = {
        text: null,
        filePath: null,
        relativePath: null,
  ...
  private writeLockFiles(): void {
    const data: LockFile = {
      ...
      serverAuthToken: this.authToken,
    };
