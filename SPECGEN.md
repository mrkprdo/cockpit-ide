...
Every part of a spec file belongs to one of two classes. Tools that rewrite specs (the SpecsMap reconciler) may only touch **structural** fields; **contract** prose is written by humans or agents and is never machine-rewritten.

| Class | Fields / sections | Who writes |
|-------|-------------------|------------|
| **Structural** | Frontmatter `file`, `entry`, `exports`; `## Dependencies`, `## Referenced By`, `## IPC Channels`, `## External Dependencies`, `## Test`; `main.spec.md` Features table rows | Reconciler or bootstrap skeleton |
| **Contract** | Description paragraph, `## Interface`, `## State`, `## Lifecycle`, `## Scripts`, `## Build`, all UI sub-spec sections; `main.spec.md` Stack / Architecture / IPC Channels catalog / Keyboard Shortcuts / Test Coverage | Human or agent only |

Unknown `##` sections are **contract-adjacent**: any rewrite must preserve them byte-for-byte.

Frontmatter `name`, `type`, `layer`, `singleton` are structural at skeleton-creation time only; once a spec exists, reconcile never overwrites them.

### Canonical link form

Dependency and reference items use the plain form:

```
- **<feature-name>** `<file-path>` — <usage>
```

Wiki-link form (`[[spec.md|name]]`) is accepted on parse for backward compatibility but serializers emit the plain form.

## Reconcile Modes

| Mode | Behavior |
|------|----------|
| `report` | Compute the proposed structural diff; write nothing |
| `structural` | Patch structural fields only; optionally create skeleton specs for unspecced files; recompute all `## Referenced By` globally from Dependencies |
| `full` | Explicit confirm only; still must not drop unknown sections or replace non-stub descriptions |

Default for explicit user/agent action: `structural`. Watch-triggered suggestions: `report` only. Skeleton descriptions use the stub `TODO: describe this feature` and are flagged by validation until replaced.
...
| `spec.missing-file` | error | `file:` / `entry:` path missing on disk |
...
The SpecsMap graph cache (`.cockpit/specsmap-*.json`) is **cache only** — on-disk `*.spec.md` is always canonical. Snapshot v2 schema:
...
```json
{
  ...
  "corpusHash": "hash over sorted path+content of all specs",
  "builtAt": "ISO-8601",
  "nodes": []
}
```
